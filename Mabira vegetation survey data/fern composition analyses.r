...
keep3
ccafernfull<-cca(ferns[keep3,colSums(ferns>0, na.rm=T)>1]~Forest+Slope+BA+Deadwood+Stumps+LLCover+LLDepth+ShrubCover+pH+N.+C.+TCat+CanO+m.from.stream,env[keep3,])
ccafernfull
anova(ccafernfull)
plot(ccafernfull)
...
## straight ordistep reports m from stream, slope, forest:3.6467 Constrained    0.5797
...
#now just forest
ccafernjustforest<-cca(ferns[keep3,colSums(ferns>0, na.rm=T)>1]~Forest,env[keep3,])
...
ccafernmodel<-cca(ferns[keep3,colSums(ferns>0, na.rm=T)>1]~Forest+pH+Slope,env[keep3,])
ccafernmodel #Constrained  
vif.cca(ccafernmodel) #highest is 1.32 (forest)


ccafernmodelpart<-cca(ferns[keep3,colSums(ferns>0, na.rm=T)>1]~pH+Slope+Condition(Forest),env[keep3,])
ccafernmodelpart
#Constrained   
...
ccaferns.FpluspH<- cca(ferns[keep3,colSums(ferns>0, na.rm=T)>1]~Forest+pH,env[keep3,])
ccaferns.FpluspH
# xxx; xxx%
ccaferns.FpartpH<- cca(ferns[keep3,colSums(ferns>0, na.rm=T)>1]~Forest+Condition(pH),env[keep3,])
ccaferns.FpartpH
# Conditional xxx; xxx%. Constrained xxx; xxx%
ccaferns.pHpartF<- cca(ferns[keep3,colSums(ferns>0, na.rm=T)>1]~pH+Condition(Forest),env[keep3,])
ccaferns.pHpartF
# Conditional xxx; xxxx%. Constrained xxx; xxx%
ccaferns.FplusA<- cca(ferns[keep3,colSums(ferns>0, na.rm=T)>1]~Forest+aspect,env[keep3,])
ccaferns.FplusA
#xxx; xxx%
ccaferns.FpartA<- cca(ferns[keep3,colSums(ferns>0, na.rm=T)>1]~Forest+Condition(aspect),env[keep3,])
ccaferns.FpartA
# Conditional xxx; xxx%. Constrained xxx; xxx%
ccaferns.ApartF<- cca(ferns[keep3,colSums(ferns>0, na.rm=T)>1]~aspect+Condition(Forest),env[keep3,])
ccaferns.ApartF
# Conditional xxx; xxx%. Constrained xxx; xxx%
...
# Conditional xxx; xxx%. Constrained xxx; xxx%
#threeway model
ccafern.FplusApluspH<-cca(ferns[keep3,colSums(ferns>0, na.rm=T)>1]~Forest+pH+aspect,env[keep3,])
ccafern.FplusApluspH
#xxxx; xxxx%


#full end model
ccafern.FplusApluspHplusM<-cca(ferns[keep3,colSums(ferns>0, na.rm=T)>1]~Forest+pH+Aspect+m.from.stream,env[keep3,])

#xxxx; xxxx%



keep<-!is.na(env$Forest)&!is.na(ferns[,1])
library(indicspecies)
fernpatt<-multipatt(ferns[keep,], env$Forest[keep], func="r")
summary(fernpatt)


#making the bar plots of species habitat affiliation - remember to run keep3 from line 8 above!!!!
temp<-merge(ferns[keep3,],env$Forest, by="row.names") 

...
ccasubcanopywoodyfull<-cca(sqrt(subcanopywoody[keep6,colSums(subcanopywoody>0, na.rm=T)>1])~Forest+Slope+BA+Deadwood+Stumps+LLCover+LLDepth+pH+N.+C.+TCat+CanO+m.from.stream,env[keep6,])
ccasubcanopywoodyfull
anova(ccasubcanopywoodyfull)

plot(ccasubcanopywoodyfull, type="n")
points(ccasubcanopywoodyfull, display="sites", col=as.numeric(env$Forest[keep6])+1, pch=16)
...
points(ccawithnothingin, display="sites", col=as.numeric(env$Forest[keep6])+1, pch=16)
...
# ordistep keeps Forest, lldepth and  TCAT
...
# chooses null model 
#now just forest
ccasubcanopywoodyjustforest<-cca(sqrt(subcanopywoody[keep6,colSums(subcanopywoody>0, na.rm=T)>1])~Forest,env[keep6,])
...
ccasubcanopywoody.FplusTCatplusLLDepth<- cca(sqrt(subcanopywoody[keep6,colSums(subcanopywoody>0, na.rm=T)>1])~Forest+TCat+LLDepth,env[keep6,])
ccasubcanopywoody.FplusTCatplusLLDepth
# Constrained    
ccasubcanopywoody.FpartTCat<- cca(sqrt(subcanopywoody[keep6,colSums(subcanopywoody>0, na.rm=T)>1])~Forest+Condition(TCat),env[keep6,])
ccasubcanopywoody.FpartTCat
# Constrained  
ccasubcanopywoody.TCatplusLLDepthpartF<- cca(sqrt(subcanopywoody[keep6,colSums(subcanopywoody>0, na.rm=T)>1])~TCat+LLDepth+Condition(Forest),env[keep6,])
...
points(ccasubcanopywoody.FplusTCatplusLLDepth, display="sites", col=colour[env$Forest[keep6]], pch=16)
...
scwpatt<-multipatt(subcanopywoody[keep,], env$Forest[keep], func="r")
summary(scwpatt)


#remember to run keep6 from line 4 above!!!!
temp<-merge(subcanopywoody[keep6,],env$Forest, by="row.names") 
temp
str(temp)

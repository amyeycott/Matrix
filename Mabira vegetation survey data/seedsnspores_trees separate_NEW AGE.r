#Data loading & cleaning 2 - 44 in seedsnspores.r. There is no use of age class in the data loading/cleaning steps
...
#better to split in two stages   F
...
#total area sampled - 39 transects, 10 samples, each of 2.6cm diameter. Each core is 5.31 cm2 and 26.55cm3. Each transect is 53.1cm2, total area is 2070.9cm2. Per transect volume is Top 1cm 53.1cm3, bottom  212.4cm3, sum 265.5. total vol of all top layers is 2070.9cm3, bottom 4cm makes 8283.6cm3, total volume sampled 10354.5cm3 (about ten litres).  
...
boxplot((all.layers.dens[,c(19,20,3,4,11,12)])*1000+0.1, at=c(1,2,4,5,7,8),names=c("Fern spores 0-1 cm", "Fern spores 1-5 cm","Herb/shrub seeds 0-1 cm", "Herb/shrub seeds 1-5 cm", "Tree seeds 0-1 cm", "Tree seeds 1-5 cm"),  log="y",ylab=expression(paste("Density litre"^-1)), cex.axis=0.75,  las=2)  #only logs if you give it plus zero point one, which breaks the scale unless I can find out how to take away the decimal place. Also, is a log boxplot weird?
boxplot((all.layers.dens[,c(19,20,3,4,11,12)])*1000, at=c(1,2,4,5,7,8),names=c("Fern spores 0-1 cm", "Fern spores 1-5 cm","Herb/shrub seeds 0-1 cm", "Herb/shrub seeds 1-5 cm", "Tree seeds 0-1 cm", "Tree seeds 1-5 cm"),  ylab=expression(paste("Density litre"^-1)), cex.axis=0.75,  las=2, outline=F) 
savePlot("split-seed and spore density litre by layer NEWAGE.emf", type="emf")
x11(); par(mar=c(5,5,2,1.4), mfrow=c(1,3))
boxplot((all.layers.dens[,c(19,20)])*1000, names=c("0-1", "1-5"),  ylab=expression(paste("Density litre"^-1)), xlab="depth cm",main="Fern spores", ylim=c(0,250), cex.axis=1.5, cex.lab=1.5, outline=F)
boxplot((all.layers.dens[,c(3,4)])*1000, names=c("0-1", "1-5"),  ylab=expression(paste("Density litre"^-1)), xlab="depth cm",main="Herb & shrub seeds", ylim=c(0,100), cex.axis=1.5, cex.lab=1.5, outline=F)
boxplot((all.layers.dens[,c(11,12)])*1000,names=c("0-1", "1-5"),  ylab=expression(paste("Density litre"^-1)), xlab="depth cm",main="Tree seeds", ylim=c(0,100), cex.axis=1.5, cex.lab=1.5, outline=F)
savePlot("Esaete et al Figure 1 depth boxplot.eps", type="eps")
...
byage<-merge(byage, env$newage, by.x=1, by.y=0)
rownames(byage)<-byage$Row.names

byage$spores.densm2<-byage$spores.x/53.1#this makes no sense. This surely gives per cm2?
byage$herbseeds.densm2<-byage$herbseeds.x/53.1
byage$treeseeds.densm2<-byage$treeseeds.x/53.1
table(byage$y[!is.na(byage$herbseeds.x)])


#six plots by age: note fucked with for ppt
x11(point=16, height=6, width=6); par(mfrow=c(2,3), mar=c(2,2.5,2.5,1.2), font.main=1, cex.main =0.8, cex.axis=0.8)
boxplot(byage$treeseeds.densm2*1000~byage$y, at=c(3,2,1), main=expression(paste("Tree seeds per ", m^2," *")), ylim=c(0,300), names=c("OG","MS", "YS"))
boxplot(byage$herbseeds.densm2*1000~byage$y, at=c(3,2,1), main=expression(paste("Herb/shrub seeds per ", m^2," ns")), ylim=c(0,300), names=c("OG","MS", "YS"))
boxplot(byage$spores.densm2*1000~byage$y, at=c(3,2,1), main=expression(paste("Fern spores per ", m^2," ns")), ylim=c(0,1000), names=c("OG","MS", "YS") )
boxplot(byage$treeseeds.y~byage$y, at=c(3,2,1), main="Tree seed species richness ns", ylim=c(0,4), yaxt="n", names=c("OG","MS", "YS"))
axis(side=2, at=c(0,2,4))
boxplot(byage$herbseeds.y~byage$y, main="Herb/shrub seed species richness ns", ylim=c(0,4), yaxt="n", at=c(3,2,1), names=c("OG","MS", "YS"))
axis(side=2, at=c(0,2,4))
boxplot(byage$spores.y~byage$y, at=c(3,2,1), main="Fern spore species richness ns", ylim=c(0,4), yaxt="n", names=c("OG","MS", "YS"))
axis(side=2, at=c(0,2,4))
savePlot("Esaete et al Figure 2 Forest age for ppt.eps", type="eps")
savePlot("Esaete et al Figure 2 Forest age for ppt.emf", type="emf")


#by(byage, byage$y, FUN=mean(byage$herbseeds.x/(byage$herbseeds.x+byage$treeseeds.x+byage$ 
...
kruskal.test(byage$herbseeds.x, byage$y)  #abun Kruskal-Wallis chi-squared = 1.1305, df = 2, p-value = 0.5682
kruskal.test(byage$treeseeds.x, byage$y)  #Kruskal-Wallis chi-squared = 7.4326, df = 2, p-value = 0.02432
kruskal.test(byage$spores.x, byage$y)     #Kruskal-Wallis chi-squared = 0.6374, df = 2, p-value = 0.7271
kruskal.test(byage$herbseeds.y, byage$y)  #rich #Kruskal-Wallis chi-squared = 2.0944, df = 2, p-value = 0.3509
kruskal.test(byage$treeseeds.y, byage$y)  #Kruskal-Wallis chi-squared = 2.9623, df = 2, p-value = 0.2274
kruskal.test(byage$spores.y, byage$y)     #Kruskal-Wallis chi-squared = 0.2038, df = 2, p-value = 0.9031

#x11(); par(mfrow=c(2,1))
#plot (jitter(abunss$spores, amount = 0.1)~jitter(abunss$seeds, amount=0.01), col=as.factor(env$newage), xlab="abun_seeds", ylab="abun_spores")
#legend(locator(1),c("og","os","ys"),pch=c(1,1,1),col=c(1,2,3))
#plot (jitter(richss$spores, amount = 0.1)~jitter(richss$seeds, amount=0.01), col=as.factor(env$newage), xlab="spprichseeds", ylab="spprichspores")
#legend(locator(1),c("og","os","ys"),pch=c(1,1,1),col=c(1,2,3))
...
x11();par(mfrow=c(2,3))
mapply( function(x,n)hist(x, main= n),x=list(abunss$herbseeds, abunss$treeseeds, abunss$spores, richss$herbseeds, richss$treeseeds, richss$spores),n=c("herb seed abundance", "tree seed abundance", "spore abundance", "herb seed species richness","tree seed species richness","fern spore species richness"))
...
plot((spaccfern$sites*300),spaccfern$richness, col="black", type="l", ylab="Cumulative species richness",xlab=expression(paste("Area sampled m"^2)), log="xy", xlim=c(0.001,20000), ylim=c(1,50), lwd=2, cex.axis=1.3, cex.lab=1.3)          #ylab=expression(paste("Density cm"^-3))
...
lines((spaccspore$sites*0.00531),spaccspore$richness, col="blue", lwd=2)
lines((spaccherbseed$sites*0.00531),spaccherbseed$richness, col="blue", lty=2, lwd=2)
lines((spacctreeseed$sites*0.00531),spacctreeseed$richness, col="blue", lty=3, lwd=2)
                 
legend("bottomright",c("Ferns","Herbs/shrubs","Trees", "Fern spores","Herb/shrub seeds","Tree seeds"),lty=c(1,2,3,1,2,3),col=c("black","black","black","blue","blue", "blue"), lwd=2, cex=1.3)
savePlot("Esaete et al S3 species accumulation.eps", type="eps")
...
splitscores<-merge(env$newage,herbseed.sitescores, by.x=0, by.y=0, all=T)
...
apply(splitscores[,3:14],2,function(x){kruskal.test(x[!is.na(x)], splitscores$x[!is.na(x)])})
# sig AXIS1 herbseeds, ferns, trees, sig axis 2 veg 
...
x11();par(mfrow=c(2,3))
plot(splitscores$herbseed.ax2[!is.na(splitscores$herbseed.ax2)]~splitscores$herbseed.ax1[!is.na(splitscores$herbseed.ax1)], col=as.factor(splitscores$newage[!is.na(splitscores$herbseed.ax1)]))#pretty bad - six or so outliers
plot(splitscores$treeseed.ax2[!is.na(splitscores$treeseed.ax2)]~splitscores$treeseed.ax1[!is.na(splitscores$treeseed.ax1)], col=as.factor(splitscores$newage[!is.na(splitscores$treeseed.ax1)]))#really quite dreadful - max axis 1 score is 300, next highest is 10
plot(splitscores$spore.ax2[!is.na(splitscores$spore.ax2)]~splitscores$spore.ax1[!is.na(splitscores$spore.ax1)], col=as.factor(splitscores$newage[!is.na(splitscores$spore.ax1)])) # fine - how is it managing to be fine on two axes with such little information???
plot(splitscores$floorveg.ax2[!is.na(splitscores$floorveg.ax2)]~splitscores$floorveg.ax1[!is.na(splitscores$floorveg.ax1)], col=as.factor(splitscores$newage[!is.na(splitscores$floorveg.ax1)]))#very nice plot
plot(splitscores$trees.ax2[!is.na(splitscores$trees.ax2)]~splitscores$trees.ax1[!is.na(splitscores$trees.ax1)], col=as.factor(splitscores$newage[!is.na(splitscores$trees.ax1)]))# also just fine
plot(splitscores$ferns.ax2[!is.na(splitscores$ferns.ax2)]~splitscores$ferns.ax1[!is.na(splitscores$ferns.ax1)], col=as.factor(splitscores$newage[!is.na(splitscores$ferns.ax1)])) #one crazy outlier, the rest are fine and an age effect is plausible
...
#leveneTest(sco.dcaseeds$DCA1, sco.dcaseeds$newage)
#leveneTest(sco.dcaseeds$DCA2, sco.dcaseeds$newage)
#leveneTest(sco.dcaseeds$NMDS1, sco.dcaseeds$newage)
#leveneTest(sco.dcaseeds$NMDS2, sco.dcaseeds$newage)
#all of these pass levene test

#however you do the test, the following relationships are ns
#kruskal.test(sco.dcaseeds$DCA1, sco.dcaseeds$newage)
#lm.DCA1<-lm(sco.dcaseeds$DCA1~sco.dcaseeds$newage)
#summary(lm.DCA1)
#kruskal.test(sco.dcaseeds$DCA2, sco.dcaseeds$newage)
#lm.DCA2<-lm(sco.dcaseeds$DCA2~sco.dcaseeds$newage)
#summary(lm.DCA2)
#kruskal.test(sco.dcaseeds$NMDS1, sco.dcaseeds$newage)
#lm.NMDS1<-lm(sco.dcaseeds$NMDS1~sco.dcaseeds$newage)
#summary(lm.NMDS1)
#kruskal.test(sco.dcaseeds$NMDS2, sco.dcaseeds$newage)
#lm.NMDS2<-lm(sco.dcaseeds$NMDS2~sco.dcaseeds$newage)
#summary(lm.NMDS2)
...
herbsforcrusty<-metaMDS(floorveg.red)#borked
herbseedsforcrusty<-metaMDS(herbseed.red)
treesforcrusty<-metaMDS(trees.red)
treeseedsforcrusty<-metaMDS(treeseed.red)
fernsforcrusty<-metaMDS(ferns.red)
fernsporesforcrusty<-metaMDS(spores.red)
...
#kruskal.test(distances2$distance.values, distances2$newage)
#tapply(distances2$distance.values, distances2$newage, FUN=median)
...
forplotherbnseedsums<-herbnseed.sums
is.na(forplotherbnseedsums) <- !forplotherbnseedsums
forplotsporefernsums<-sporefernsums
is.na(forplotsporefernsums) <- !forplotsporefernsums
forplottreenseedsums<-treenseed.sums
is.na(forplottreenseedsums) <- !forplottreenseedsums

x11()
barplot(t(as.matrix(herbnseed.sums[,c(3,4)])), beside=T, horiz=T,  xlab="Proportion of total", xlim=c(0,0.3), yaxs="i", yaxt="n", las=2, cex.names=0.7, main="Herb and shrub species") #make the biggest graph
ylim0<-par()$usr[3:4] #find out how long the axis is and make that the axis length for all subsequent graphs.

 
x11();par(mar=c(4,13,3,1), cex=0.7, xpd=T)
barplot(t(as.matrix(forplotherbnseedsums[,c(3,4)])), beside=T, horiz=T,  xlab="Percentage of total", xlim=c(0,0.3),ylim=ylim0, xaxt = "n",las=2, font.axis = 3, cex.names=0.8) #this likes a matrix. x is supressed so that y (the trickier axis to draw manually) can be in italics. 
...
axis(1, font.axis=1, labels=c(0,10,20,30), at=c(0,0.1,0.2,0.3)) #puts the x axis in without italics and turns it into %
savePlot("split herbsnshrubs ingerplot.png", type="png") 
savePlot("Esaete et al S2i herbs and shrubs.eps", type="eps")
# only 2 shared species:  Pseuderanthemum ludovicianum, Solanum mauritianum. Note that ferns and trees have 4 and 3 shared species respectively.  

barplot(t(as.matrix(forplottreenseedsums[,c(3,4)])),beside=T, horiz=T,  xlab="Percentage of total", xlim=c(0,0.3),ylim=ylim0, xaxt="n", las=2, font.axis=3, cex.names=0.8) 
legend(x=0.2, y=150, legend=c("in vegetation (abundance)","in seedbank"), fill=colour) 
#locator(type = "n")
text(0.1,165 ,"Tree species", cex=1.3)
text(0.29,20,(round(max(treenseed.sums[,c(3,4)]), digits=2))*100)
arrows(0.295, 20, 0.305, 20, length = 0.075, code=2)
axis(1, font.axis=1, labels=c(0,10,20,30), at=c(0,0.1,0.2,0.3))
savePlot("split treesseeds ingerplot.png", type="png")
savePlot("Esaete et al S2ii trees.eps", type="eps") 
barplot(t(as.matrix(forplotsporefernsums[,c(3,4)])), beside=T, horiz=T,  xlab="Percentage of total", xlim=c(0,0.3), ylim=ylim0, xaxt="n", las=2, font.axis=3, cex.names=0.8)
legend(x=0.16, y=95, legend=c("in vegetation (relative abundance)","in soil spore bank"), fill=colour)
#locator(type = "n")
text(0.1,105,"Fern species", cex=1.3) 
text(0.29,20,(round(max(sporefernsums[,c(3,4)]), digits=2))*100)
arrows(0.295, 20, 0.305, 20, length = 0.075, code=2)
axis(1, font.axis=1, labels=c(0,10,20,30), at=c(0,0.1,0.2,0.3))
savePlot("split sporesnferns ingerplot.png", type="png") 
savePlot("Esaete et al S2iii ferns.eps", type="eps") 
...
seedsgrass_forhabit<-seedsgrass_forhabit[!is.na(seedsgrass_forhabit$newage),]
seedsgrass_forhabit<-rowSums(rowsum(seedsgrass_forhabit[2:6],seedsgrass_forhabit$newage, na.rm=T))
seedsShrub_forhabit<-merge(seedsShrubs, env, by=0)
seedsShrub_forhabit<-seedsShrub_forhabit[!is.na(seedsShrub_forhabit$newage),]
seedsShrub_forhabit<-rowSums(rowsum(seedsShrub_forhabit[2:3],seedsShrub_forhabit$newage, na.rm=T))
seedsNonwoody_forhabit<-merge(seedsNonwoody, env, by=0)
seedsNonwoody_forhabit<-seedsNonwoody_forhabit[!is.na(seedsNonwoody_forhabit$newage),]
seedNonwoody_forhabit<-rowSums(rowsum(seedsNonwoody_forhabit[2:18],seedsNonwoody_forhabit$newage, na.rm=T))
seedsTrees_forhabit<-merge(seedsTrees, env, by=0)
seedsTrees_forhabit<-seedsTrees_forhabit[!is.na(seedsTrees_forhabit$newage),]
seedTrees_forhabit_noBp<-rowSums(rowsum(seedsTrees_forhabit[3:6],seedsTrees_forhabit$newage, na.rm=T))
seedTrees_forhabit_Bp<-rowSums(rowsum(seedsTrees_forhabit[2],seedsTrees_forhabit$newage, na.rm=T))
sums_forhabit<-cbind(seedsgrass_forhabit, seedsShrub_forhabit, seedNonwoody_forhabit,  seedTrees_forhabit_noBp,seedTrees_forhabit_Bp )
colSums(rowSums(sums_forhabit))
(sums_forhabit)/(rowSums(sums_forhabit))*100
colSums(sums_forhabit)/sum(colSums(sums_forhabit))*100

x11();par(xpd=T, cex=0.75)
barplot(t((sums_forhabit)/(rowSums(sums_forhabit))*100), xlim=c(0,1), width=0.2, space=0.1, names.arg=c("Older","Mid","Younger"), ylab="Percent of total seeds", col=c("grey15","grey35", "grey55", "grey75","grey75"))
text(0.12, -8, labels="growth")
text(0.34, -8, labels="secondary")
text(0.56, -8, labels="secondary")
text(0.34, -12, labels="Stand age")
barplot(t((sums_forhabit)/(rowSums(sums_forhabit))*100),add=T, xlim=c(0,1), width=0.2, space=0.1, ylab="Percent of total seeds", col="black",density = c(0,0,0,0,20), angle = 45, border=NA, xaxt="n")
legend(0.71,100,fill=c("grey75","grey75","grey55","grey35", "grey15"), legend=c(expression(italic("Broussonetia papyrifera")),"Other trees","Herbs","Shrubs","Grasses"), bty="n")
polygon((c(0.73,0.73,0.746,0.746)),(c(96.7,98,98,96.7)),col="black", density=25, angle=45, border = NA)
savePlot("Esaete et al Fig 3 plant groups.eps", type="eps")

#redraw for ppt
x11(7,5);par(xpd=T)
barplot(t((sums_forhabit)/(rowSums(sums_forhabit))*100), xlim=c(0,1), width=0.2, space=0.1, names.arg=c("Older","Mid","Younger"), ylab="Percent of total seeds", col=c("grey15","grey35", "grey55", "grey75","grey75"))
text(0.12, -17, labels="growth")
text(0.34, -17, labels="secondary")
text(0.56, -17, labels="secondary")
text(0.34, -27, labels="Stand age")
barplot(t((sums_forhabit)/(rowSums(sums_forhabit))*100),add=T, xlim=c(0,1), width=0.2, space=0.1, ylab="Percent of total seeds", col="black",density = c(0,0,0,0,20), angle = 45, border=NA, xaxt="n")
legend(0.71,100,fill=c("grey75","grey75","grey55","grey35", "grey15"), legend=c(expression(italic("Broussonetia papyrifera")),"Other trees","Herbs","Shrubs","Grasses"), bty="n")
polygon((c(0.74,0.74,0.76,0.76)),(c(92,95,95,92)),col="black", density=25, angle=45, border = NA)
savePlot("Esaete et al Fig 3 plant groups_pptsizing.eps", type="eps")
savePlot("Esaete et al Fig 3 plant groups_pptsizing.emf", type="emf")

sporesbyage<-as.data.frame(aggregate(byage$spores.x, by=list(byage$y),FUN=sum))
sporesbyage$cristella<- aggregate(spore74$`Christella parasitica`, by=list(env$newage), function(x){sum(x, na.rm=TRUE)})[2]
barplot(c(100,100,100), ylab="percent total spores", xlim=c(0,1), width=0.2, space=0.1, names.arg=c("Younger","Mid","Older"))
barplot(c(94,96,94),add=T, xlim=c(0,1), width=0.2, space=0.1, col="black",density = 25, angle = 45, border=NA, xaxt="n")
legend("topright", col="black",density = 25, angle = 45, border=NA, legend=expression(italic("Cristella parasitica")))
text(0.12, -17, labels="secondary")
text(0.34, -17, labels="secondary")
text(0.56, -17, labels="growth")
text(0.34, -27, labels="Stand age")
savePlot("itsallcristella.emf", type="emf")
...
legend(7, 3, legend=unique(fortraits$Dispersal), pch=unique(as.numeric(fortraits$Dispersal)), col=1)
savePlot("seedsize_vs_abun.emf", type="emf")
cor.test(log(fortraits$Total),fortraits$seed_size_mm) #rho=0,16, P=0.382  spearman is also ns
...
abline(0, 0)
 ...
 unique(thinspp$Transect_id[thinspp$Species_Name=="Pseuderanthemum ludovicianum"])